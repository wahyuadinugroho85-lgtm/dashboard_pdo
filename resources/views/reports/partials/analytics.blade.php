...
<div id="tab-analytics" class="tab-content block w-full space-y-6">
    <div id="analytics-container" class="transition-all duration-300 rounded-xl relative">
        ...
        @php
            // PERBAIKAN: Menambahkan nilai fallback '?? 0' agar tidak crash jika datanya kosong
            $valBgt = $gCostPalmOilBgt ?? 0;
            $valReal = $gCostPalmOilReal ?? 0;
            $gPctCostOil = $valBgt > 0 ? ($valReal / $valBgt) * 100 : 0;
        @endphp
        ...
        <div id="dynamic-dashboard-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <!-- 4. WIDGET MUTASI / TURNOVER KARYAWAN -->
            <div id="w-mutasi" data-wname="Mutasi Karyawan" class="widget-item col-span-1 group relative bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex items-center justify-between border-l-4 border-l-orange-500 hover:shadow-md transition-all duration-300 cursor-help">
                <div>
                    <p class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1 flex items-center gap-1">Mutasi Karyawan</p>
                    @php
                        $gKeluarSbi = 0;
                        ...
                        $gPctKeluarSbi = $cPctKeluarSbi > 0 ? $tPctKeluarSbi / $cPctKeluarSbi : 0;
                    @endphp
                    <h3 class="text-2xl font-extrabold text-slate-800">{{ number_format($gKeluarSbi, 0) }} <span class="text-sm font-bold text-slate-500">TK Keluar</span></h3>
                    <p class="text-xs font-bold text-orange-600 mt-1">Rate Keluar: {{ number_format($gPctKeluarSbi, 2) }}% (Sbi)</p>
                </div>
                ...
                <div class="absolute right-0 top-[105%] tooltip-table z-[100] opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-200 pointer-events-none group-hover:pointer-events-auto">
                    <div class="tooltip-box bg-slate-800 text-slate-100 text-xs rounded-lg shadow-xl p-4 border border-slate-700 whitespace-nowrap min-w-max">
                        <div class="font-bold text-orange-300 mb-2 border-b border-slate-600 pb-1 text-sm tracking-wide">Detail Mutasi (Turnover) per PT Sbi</div>
                        <table class="w-full text-right">
                            <tr class="text-slate-400 border-b border-slate-600">
                                <th class="text-left pb-1 pr-3">PT</th>
                                <th class="pb-1 pl-4 text-emerald-400">Masuk Sbi</th>
                                <th class="pb-1 pl-4 text-rose-400">Keluar Sbi</th>
                                <th class="pb-1 pl-4">% Keluar Sbi</th>
                            </tr>
                            @foreach($estates as $estate)
                                @php 
                                    $masukSbi = isset($dataMatrix[$estate->kode]['pekerja_sbi']['Mutasi']) ? $dataMatrix[$estate->kode]['pekerja_sbi']['Mutasi']->where('sub_kategori', 'Masuk (Bi)')->sum('jumlah_tk') : 0;
                                    $keluarSbi = isset($dataMatrix[$estate->kode]['pekerja_sbi']['Mutasi']) ? $dataMatrix[$estate->kode]['pekerja_sbi']['Mutasi']->where('sub_kategori', 'Keluar (Bi)')->sum('jumlah_tk') : 0;
                                    $pctSbi = isset($dataMatrix[$estate->kode]['pekerja']['Mutasi']) ? ($dataMatrix[$estate->kode]['pekerja']['Mutasi']->firstWhere('sub_kategori', '% Keluar Sbi')->persentase ?? 0) : 0;
                                @endphp
                                <tr class="border-b border-slate-700 last:border-0">
                                    <td class="py-1.5 text-left font-medium pr-3">{{ $estate->kode }}</td>
                                    <td class="pl-4 font-bold text-white">{{ number_format($masukSbi, 0) }}</td>
                                    <td class="pl-4 font-bold text-white">{{ number_format($keluarSbi, 0) }}</td>
                                    <td class="pl-4 font-bold text-orange-400">{{ number_format($pctSbi, 2) }}%</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
